...
Music commands (members must be in a voice channel):
...
YouTube is the playback source. YouTube playlists are expanded into the queue. Spotify and Apple Music single-track links are resolved to matching YouTube audio; Spotify and Apple Music playlist links require their platform API credentials and are not expanded automatically. Railway installs system `ffmpeg` and `yt-dlp` through `nixpacks.toml` for music playback.
