...
async function resolveMusicQuery(input) {
    ...
    if (play.yt_validate(value) === 'playlist') {
        ...
        return videos.filter(video => video.url).slice(0, 100).map(video => ({ title: video.title, url: video.url, duration: video.durationRaw, requested: value }));
    }
    let query = value;
    const isExternalTrack = /open\.spotify\.com|music\.apple\.com/i.test(value);
    ...
    if (isExternalTrack) {
        try {
            ...
            if (title) query = title.replace(/\s*[-|]\s*(Spotify|Apple Music).*$/i, '').trim();
        } catch {
            query = value;
        }
    }

    const results = await play.search(query, { limit: 1, source: 'youtube' });
    const track = results[0];
    if (!track) throw new Error('No playable result found.');
    return [{ title: track.title, url: track.url, duration: track.durationRaw, requested: value }];
}
...
async function playNext(guildId) {
    ...
    try {
        const source = await play.stream(item.url, { quality: 2, discordPlayerCompatibility: true });
        queue.current = item;
        queue.playing = true;
        queue.player.play(createAudioResource(source.stream, { inputType: source.type }));
        await queue.textChannel?.send({ embeds: [card('Now playing', `**${item.title}**\nRequested by <@${item.requesterId}>`, COLORS.cyan).addFields({ name: 'Length', value: item.duration || 'Unknown', inline: true })] });
    } catch (error) {
        console.error('[music] stream failed:', error.message);
        await queue.textChannel?.send(`I could not play **${item.title}**. Skipping to the next track.`).catch(() => undefined);
...
async function playMusic(message, input) {
    ...
    const tracks = await resolveMusicQuery(input);
...
client.on('messageCreate', async message => {
    ...
    try {
        ...
        if (command === 'play') return playMusic(message, args);
        if (command === 'stop') return stopMusic(message);
        if (command === 'skip') {
            const queue = musicQueues.get(message.guildId);
            if (!queue?.current) return message.reply('Nothing is currently playing.');
            queue.player.stop();
            return message.reply({ embeds: [card('Skipped', `Skipped **${queue.current.title}**.`, COLORS.amber)] });
        }
        if (command === 'pause' || command === 'resume') {
            const queue = musicQueues.get(message.guildId);
            if (!queue?.current) return message.reply('Nothing is currently playing.');
            if (command === 'pause') queue.player.pause();
            else queue.player.unpause();
            return message.reply(`Music ${command}d.`);
        }
        if (command === 'queue') {
            const queue = musicQueues.get(message.guildId);
            if (!queue?.current && !queue?.items.length) return message.reply('The music queue is empty.');
            const upcoming = queue.items.slice(0, 10).map((item, index) => `${index + 1}. **${item.title}**`).join('\n') || 'No upcoming tracks.';
            return message.reply({ embeds: [card('Music queue', queue.current ? `Now playing: **${queue.current.title}**` : 'Nothing is playing.', COLORS.cyan).addFields({ name: 'Up next', value: upcoming })] });
        }
