...
function getAudioUrl(url) {
    return new Promise((resolve, reject) => {
        const extractor = spawn('yt-dlp', ['--get-url', '--format', 'bestaudio/best', '--no-playlist', '--no-warnings', url], { windowsHide: true });
        let output = '';
        let errors = '';
        ...
        extractor.once('close', code => {
            if (code !== 0 || !output.trim()) return reject(new Error(errors.trim() || `yt-dlp exited with ${code}`));
...
async function playNext(guildId) {
    ...
    try {
        const directUrl = await getAudioUrl(item.url);
        if (!directUrl) throw new Error('yt-dlp returned no audio URL');
        const ffmpeg = spawn('ffmpeg', ['-hide_banner', '-loglevel', 'error', '-i', directUrl, '-vn', '-f', 's16le', '-ar', '48000', '-ac', '2', 'pipe:1'], { windowsHide: true });
