...
async function playNext(guildId) {
    ...
    try {
        const extractor = spawn('yt-dlp', ['--no-playlist', '--no-warnings', '--quiet', '--format', 'bestaudio/best', '--output', '-', '--no-part', '--no-cache-dir', '--extractor-args', 'youtube:player_client=android', item.url], { windowsHide: true });
...
client.once('ready', async readyClient => {
    ...
    await player.extractors.loadMulti(DefaultExtractors).catch(error => console.error('[music] extractor setup failed:', error.message));
});

player.events.on('playerStart', (queue, track) => {
    queue.metadata?.send({ embeds: [card('Music is playing', `**${APP_NAME} is now playing music.**\n\nTrack: **${track.title}**`, COLORS.cyan)] }).catch(() => undefined);
});
player.events.on('error', (queue, error) => console.error('[music] player error:', error.message));
player.events.on('playerError', (queue, error) => console.error('[music] track error:', error.message));
...
async function handleMusicSlash(interaction) {
    ...
    if (command === 'play') {
        ...
        try {
            ...
            const result = await player.play(voiceChannel, query, { nodeOptions: { metadata: { channel: interaction.channel } } });
            return interaction.editReply(`Queued **${result.track.cleanTitle || result.track.title}**. The player will announce playback here.`);
        } catch (error) {
    ...
    try {
        if (command === 'queue') {
            const current = queue.currentTrack ? `Now playing: **${queue.currentTrack.cleanTitle || queue.currentTrack.title}**` : 'Nothing is currently playing.';
            const upcoming = queue.tracks.toArray().slice(0, 10).map((track, index) => `${index + 1}. **${track.cleanTitle || track.title}**`).join('\n') || 'No upcoming tracks.';
