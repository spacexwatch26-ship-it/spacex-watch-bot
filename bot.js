...
async function playNext(guildId) {
    ...
    try {
        const directUrl = String(await youtubeDl(item.url, { getUrl: true, format: 'bestaudio/best', quiet: true, noWarnings: true })).trim();
        if (!directUrl) throw new Error('yt-dlp returned no audio URL');
        const ffmpeg = spawn(ffmpegPath, ['-hide_banner', '-loglevel', 'error', '-i', directUrl, '-vn', '-f', 's16le', '-ar', '48000', '-ac', '2', 'pipe:1'], { windowsHide: true });
