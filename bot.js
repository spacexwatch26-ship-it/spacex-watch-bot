...
const YTDLP_PATH = path.join(__dirname, process.platform === 'win32' ? 'yt-dlp.exe' : 'yt-dlp-bin');
...
function ytDlpCommand() {
    return [YTDLP_PATH, '/root/.nix-profile/bin/yt-dlp', '/usr/bin/yt-dlp', 'yt-dlp'].find(command => command === 'yt-dlp' || fs.existsSync(command));
}

async function ensureYtDlp() {
    if (fs.existsSync(YTDLP_PATH)) return;
    console.log('[music] downloading yt-dlp runtime...');
    await YTDlpWrap.downloadFromGithub(YTDLP_PATH);
    if (process.platform !== 'win32') fs.chmodSync(YTDLP_PATH, 0o755);
    console.log('[music] yt-dlp runtime ready.');
}
