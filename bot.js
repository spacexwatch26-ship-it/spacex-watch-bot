...
async function resolveMusicQuery(input) {
    ...
    return new Promise((resolve, reject) => {
        const extractor = spawn('yt-dlp', [source, '--flat-playlist', '--playlist-end', '100', '--print', '%(title)s\t%(webpage_url)s\t%(duration_string)s', '--skip-download', '--no-warnings', '--no-cache-dir'], { windowsHide: true });
        let output = '';
        let errors = '';
