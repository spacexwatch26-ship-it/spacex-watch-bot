...
async function openModerationDM(message) {
    if (!hasPermission(message.member, PermissionsBitField.Flags.ModerateMembers)) return message.reply('You need **Moderate Members** permission.');
    ...
    const panel = card('V2 MODERATION DESK', 'This panel is private. Choose a target and reason, then select an action and duration.', COLORS.amber).addFields({ name: 'Actions', value: 'Warning • Advisory • Mute / Timeout • Kick • Ban' });
    try {
        ...
        return message.reply('I could not DM you. Enable DMs from server members and try again.');
    }
}
...
async function handleSlash(interaction) {
    if (interaction.commandName === 'help') return interaction.reply({ embeds: [card('Private V2 guide', 'Only you can see this response.', COLORS.cyan).addFields({ name: 'Private commands', value: '`/help` `/status` `/dashboard` `/stats`' }, { name: 'Server workflows', value: '`-announce` `-moderate` `-mute` `-kick` `-ban` `-ticket` `-poll`' })], ephemeral: true });
